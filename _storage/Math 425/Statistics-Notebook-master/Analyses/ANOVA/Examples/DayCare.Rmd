...
The null hypothesis is that there is no difference in the number of late children for the before, during, and after time periods.

$H_0: μ_{\text{Before the Fine}} = μ_{\text{Fine Applied}} = μ_{\text{After Fine Removed}} = ... = μ$

$H_a$: At least one $μ$ is not equal to the others.
...
anova_model <- aov(NumberofLateChildren ~ Period + Treatment + Period:Treatment, data = Later)
...
# Create the xyplot with points and lines
xyplot(NumberofLateChildren ~ as.factor(Treatment), data = Later, type = c("p", "a"))

xyplot(NumberofLateChildren ~ as.factor(Period), data = Later, type=c("p","a"), auto.key=list(corner=c(1,1)) )

# p - points (dotplot), a-> average (averages)
xyplot(NumberofLateChildren ~ Period, data = Later, groups = Treatment, type=c("p","a"), auto.key=list(corner=c(1,1)) )
...
The fine was shown to increase the number of children who were late to the daycare.
