...
My family is looking into prices for the Honda Pilot near Sacramento, California. Is there an ideal price for purchasing that make and model of car? Let's take a look at some data from various websites to see if the information will help with that decision.
...
Aside from the piecewise, the log transformation has the smallest P-Values, but the lowest $R^2$ value. If we accept p values as significant at the 0.5 level, then all the models have very significant p values, and only our $R^2$ is relevant. That doesn't seem like a meaningful way to identify the best model, though. They seem so similar that it seems hard to pick a best.

#### Buying Price

Definitely don't buy it before 25,000 miles. In looking through prices, not many Honda Pilots were for sale past about 125,000 miles, just looking around. This would suggest that they might bite the dust around then- or if somebody still has one, they are repairing it and making it last longer, but these are just guesses. We can guess that past 150,000, they might not last so well.
...
Why does the quadratic perform best? I don't know. What I do know, is that dealerships are expensive, and/or you shouldn't buy a used Honda Pilot unless you get a really good deal.
...
All plots look good for the most part.
It's noteworthy that both the log and sqrtsqrt transformations appeat to have non-constant variance, but the quad transformation has constant variance.

#### Breakpoints

Different breakpoints can be tried for the piecewise function- but it's not really science at that point. It does not affect most of the points, so it doesn't seem to really effect things all that much. Who would buy a Honda Pilot that has over 180,000 miles on it? And for $9,000?