...
We are only interested in the most recent data, so we use the `slice_max()` command. `slice_max()` is essentially a shortcut that allows you to filter by the maximum value of a column, in our case `date`.  The goal is to make it so the dataset includes data for just the most recent date.
...
*Note, when expressions inside of filter are separated by a comma it is the equivalent logic of separating each logical expression with an "&". In other words, all the criteria must be met for a row to be returned.
...
```{r joining}
coronaplot <- left_join(us_states, covid48, by = c("name" = "state"))
class(coronaplot)
```
...
(p <- ggplot(data = coronaplot) +
  ...
  theme_bw() +
  theme(axis.title = element_blank()) + # Remove x and y axis titles
  labs(fill = "Confirmed Cases",  # Legend title for fill aesthetic
       title = "Confirmed COVID-19 Cases by State",  # Plot title
       subtitle = paste("as of", coronaplot$date[1])) +  # Plot subtitle with date
  scale_fill_continuous(labels = scales::comma)  # Fix the number formatting in the legend
)
```
...
```{r labels}
p +
    geom_sf_text(aes(label = cases / 1e6), size = 3, color = "white", fontface = "bold")
```
...
```{r labels}
p +
  geom_sf_text(aes(label = format(cases/1000000, size = 1)), 
               color = "white")
```
...
```{r}
p +
  geom_text_repel(aes(label = format(cases/1000000, big.mark = ",", scientific = FALSE), 
                      size = 1), 
                  color = "white")
```
...
```{r ggrepel}
p +
  geom_text_repel(aes(label = format(cases/1e6, digits = 1),
                      x = sf_coordinates(geometry)[, "X"], 
                      y = sf_coordinates(geometry)[, "Y"]), 
                  color = "white",
                  stat = "sf_coordinates",
...
Depending on the size of your plot previewer window, some of the labels won't show up. Try zooming in or knitting the document to see the final result.
...
(p <- ggplot(data = coronaplot) +
  ...
  labs(fill = "Confirmed Cases", title = "Millions of Confirmed COVID-19 Cases by State",
       subtitle = paste("as of", coronaplot$date[1])) +
...
```{r labels_key}
p +
  geom_sf_text(aes(label = format(cases/1000000, digits = 1)), 
               color = "green")
```
